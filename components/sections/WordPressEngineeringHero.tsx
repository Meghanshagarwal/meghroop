...
const pills = ['Custom WordPress', 'Headless CMS', 'Publishing Systems', 'SEO Infrastructure', 'Content Automation', 'Performance Engineering']

export default function WordPressEngineeringHero() {
  return (
    <section
      id="wordpress-engineering-hero"
      aria-label="WordPress Engineering and Premium CMS Development"
      ...
      <div className="relative z-10 max-w-5xl mx-auto px-6 text-center">
        ...
        <motion.div {...fadeUp(0)} className="flex justify-center mb-6 sm:mb-8">
          <div className="inline-flex flex-wrap justify-center items-center gap-x-2 gap-y-1 px-3 sm:px-4 py-2 rounded-full glass text-xs sm:text-sm text-gray-300">
            ...
            <span>WordPress Engineering & CMS Infrastructure</span>
            <span className="hidden sm:inline-flex items-center gap-1.5">
              <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse" />
              <span className="text-emerald-400 text-xs">Building systems</span>
            </span>
          </div>
        </motion.div>
        ...
        >
          Publishing systems{' '}
          <br />
          <span className="gradient-text">built for the modern internet.</span>
          <br />
          Fast. Semantic. Scalable.
        </motion.h1>
        ...
        >
          WordPress is powerful. But most WordPress sites feel outdated, slow, and generic.{' '}
          <span className="text-white">We build modern WordPress experiences—headless publishing systems, SEO-optimized content infrastructure, and performance-engineered platforms.</span> Not plugins. Not templates. Engineered systems that scale.
        </motion.p>
        ...
        <motion.div {...fadeUp(0.5)} className="flex flex-col sm:flex-row items-center justify-center gap-3 sm:gap-4">
          <Link
            href="#what-we-build"
            onClick={() => trackEvent('cta_click', 'ViewContent', { label: 'explore_wordpress_engineering', location: 'hero' })}
          ...
          <Link
            href="#contact"
            onClick={() => trackEvent('cta_click', 'Contact', { label: 'build_wordpress_site', location: 'hero' })}
        ...
        >
          {[
            { value: '20+', label: 'WordPress Projects' },
            { value: '95+', label: 'Lighthouse Score' },
            { value: '0.8s', label: 'Avg FCP' },
          ].map(({ value, label }) => (
