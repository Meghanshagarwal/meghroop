...
export default function MCPInfrastructureHero() {
  return (
    <section
      id="mcp-hero"
      aria-label="MeghRoop — Premium MCP Infrastructure & Context Architecture"
    ...
    >
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        ...
        <div className="absolute bottom-1/4 left-1/2 -translate-x-1/2 w-72 h-72 bg-purple-500/15 rounded-full blur-[100px] animate-float-slow" />
      </div>
      ...
      <div className="relative z-10 max-w-5xl mx-auto px-6 text-center">
        ...
        <motion.div {...fadeUp(0)} className="flex justify-center mb-6 sm:mb-8">
          <div className="inline-flex flex-wrap justify-center items-center gap-x-2 gap-y-1 px-3 sm:px-4 py-2 rounded-full glass text-xs sm:text-sm text-gray-300">
            <Cpu size={13} className="text-cyan-400 flex-shrink-0 animate-pulse" />
            <span>AI Context & Memory Systems Engineering</span>
            <span className="hidden sm:inline-flex items-center gap-1.5">
              <span className="w-1.5 h-1.5 rounded-full bg-cyan-400 animate-ping" />
              <span className="text-cyan-400 text-xs">Production Ready</span>
            </span>
          </div>
        </motion.div>
        ...
        >
          ...
          <br />
          <span className="gradient-text">context is just guessing.</span>
        </motion.h1>
        ...
        <motion.div {...fadeUp(0.5)} className="flex flex-col sm:flex-row items-center justify-center gap-3 sm:gap-4">
          <Link
            href="#what-it-means"
            onClick={() => trackEvent('cta_click', 'ViewContent', { label: 'explore_mcp', location: 'hero' })}
          ...
          <Link
            href="#contact"
            onClick={() => trackEvent('cta_click', 'Contact', { label: 'start_mcp_project', location: 'hero' })}
        ...
        >
          {[
            { value: '100%', label: 'Grounded Data Access' },
            { value: '<10ms', label: 'Context Retrieval latency' },
            { value: '0', label: 'Leaked System Credentials' },
          ].map(({ value, label }) => (
            <div key={label} className="flex flex-col items-center gap-1">
              <span className="font-heading font-bold text-2xl gradient-text">{value}</span>
