...
export default function ShopifyEngineeringHero() {
  return (
    <section
      id="shopify-engineering-hero"
      aria-label="Shopify Engineering and Premium Ecommerce Development"
      className="relative min-h-screen flex items-center justify-center overflow-hidden grid-bg pt-16"
    >
      {/* Floating gradient orbs */}
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div className="absolute top-1/4 left-1/4 w-96 h-96 bg-emerald-600/25 rounded-full blur-[120px] animate-float" />
        <div className="absolute top-1/3 right-1/4 w-80 h-80 bg-teal-600/20 rounded-full blur-[120px] animate-float-delay" />
        ...
        <div className="absolute top-1/2 left-1/3 w-64 h-64 bg-emerald-600/10 rounded-full blur-[100px] animate-float" />
      </div>
      ...
      <div className="relative z-10 max-w-5xl mx-auto px-6 text-center">
        ...
        <motion.div {...fadeUp(0)} className="flex justify-center mb-6 sm:mb-8">
          <div className="inline-flex flex-wrap justify-center items-center gap-x-2 gap-y-1 px-3 sm:px-4 py-2 rounded-full glass text-xs sm:text-sm text-gray-300">
            ...
            <span>Premium Shopify Engineering & Ecommerce Systems</span>
            <span className="hidden sm:inline-flex items-center gap-1.5">
              <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse" />
              <span className="text-emerald-400 text-xs">Commerce built right</span>
            </span>
          </div>
        </motion.div>
        ...
        <motion.p
          {...fadeUp(0.1)}
          className="font-heading font-bold text-sm sm:text-base text-gray-500 uppercase tracking-[0.2em] sm:tracking-[0.3em] mb-3 sm:mb-4"
        >
          Shopify Engineering
        </motion.p>
        ...
        >
          Commerce built{' '}
          <br />
          <span className="gradient-text">for the modern internet.</span>
          <br />
          Fast. Intentional. Conversion-Focused.
        </motion.h1>
        ...
        >
          Premium Shopify storefronts engineered for performance and conversion.{' '}
          <span className="text-white">
            Not template stores. Not generic themes. Custom ecommerce systems built with Next.js, Hydrogen, and modern
            frontend architecture.
          </span>{' '}
          Fast checkout flows. Intelligent automation. Stores that actually make money.
        </motion.p>
        ...
        <motion.div {...fadeUp(0.5)} className="flex flex-col sm:flex-row items-center justify-center gap-3 sm:gap-4">
          <Link
            href="#what-we-build"
            onClick={() =>
              trackEvent('cta_click', 'ViewContent', { label: 'explore_shopify_engineering', location: 'hero' })
            }
          ...
          <Link
            href="#contact"
            onClick={() =>
              trackEvent('cta_click', 'Contact', { label: 'build_shopify_store', location: 'hero' })
            }
        ...
        >
          {[
            { value: '40+', label: 'Shopify Projects' },
            { value: '98+', label: 'Avg Lighthouse' },
            { value: '3.2s', label: 'Avg Conversion' },
          ].map(({ value, label }) => (
            <div key={label} className="flex flex-col items-center gap-1">
              <span className="font-heading font-bold text-2xl gradient-text-emerald">{value}</span>
